...
// The BookCard component is now defined locally to resolve the import error.
const BookCard = ({ book }) => {
  return (
      ...
      <div className="p-4 flex flex-col flex-grow">
        ...
        <div className="mt-auto">
            ...
            <div className="flex flex-col space-y-2">
                <button className="w-full bg-accent text-primary font-bold py-2 px-4 rounded-lg hover:opacity-90 transition-opacity">
                    Add to Cart
                </button>
                <button className="w-full bg-primary text-white font-bold py-2 px-4 rounded-lg hover:bg-opacity-90 transition-opacity">
                    Buy Now
                </button>
            </div>
        </div>
...
const GenrePage = () => {
  ...
  const filteredBooks = allBooks[genreName] || [];
  const formattedGenreName = genreName.replace('-', ' ');

  return (
    <div className="bg-light-bg py-20">
      <div className="container mx-auto px-4 max-w-7xl">
        <div className="bg-white p-8 md:p-12 rounded-xl shadow-md">
          <h1 className="text-4xl md:text-5xl font-extrabold text-primary mb-8 capitalize">
            {formattedGenreName} Books
          </h1>
          ...
          {filteredBooks.length > 0 ? (
            ...
            <div className="text-center py-16">
              <p className="text-xl text-text-light">
                No books found in the "{formattedGenreName}" genre yet. Check back soon!
              </p>
            </div>
          )}
        </div>
